...
- Build command (from repo root):
  ...
  If Release signing fails in a sandboxed shell (keychain access), use the compile gate instead:
  `cd App && xcodebuild -project Luxicon.xcodeproj -scheme Luxicon -destination 'generic/platform=iOS Simulator,name=iPhone 16' -configuration Debug build`
  and note that in your report — the on-device Release build happens in Task 5.
- **`App/Luxicon.xcodeproj` is gitignored** (generated by xcodegen). Never `git add` it; commit only source and asset files.
...
```bash
git add App/Sources/Views/AboutGivingView.swift
git commit -m "App: AboutGivingView giving screen"
```

(`App/Luxicon.xcodeproj` is gitignored — the regenerated project is a local artifact.)
